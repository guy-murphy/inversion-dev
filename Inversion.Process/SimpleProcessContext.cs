...
namespace Inversion.Process {
	...
	/// </remarks>
	public class SimpleProcessContext : IProcessContext {

		private bool _isDisposed;

		private readonly MemoryCache _cache;
		//private readonly Subject<IEvent> _bus;
		private readonly HashSet<IProcessBehaviour> _bus;
		...
		/// </summary>
		protected IEnumerable<IProcessBehaviour> Bus {
			get { return _bus; }
		}
		...
		public IEvent Fire(IEvent ev) {
			...
			try {
				//this.Bus.OnNext(ev);
				IEnumerable<IProcessBehaviour> eligible = this.Bus.Where(behaviour => behaviour.Condition(ev));
				foreach (IProcessBehaviour behaviour in eligible) {
					behaviour.Action(ev);
				}
